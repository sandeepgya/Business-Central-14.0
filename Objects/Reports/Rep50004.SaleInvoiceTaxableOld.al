report 50004 "Sale Invoice-Taxable Old"
{
    // 1. TRI S.S 12.07.17 - New report created for "GST" Functionality.
    DefaultLayout = RDLC;
    RDLCLayout = './Objects/RDLC/SaleInvoiceTaxableOld.rdlc';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            begin
                ...
                DetailGSTLedgerEntry.SETRANGE("Transaction Type", DetailGSTLedgerEntry."Transaction Type"::Sales);
                DetailGSTLedgerEntry.SETRANGE("Document Type", DetailGSTLedgerEntry."Document Type"::Invoice);
                DetailGSTLedgerEntry.SETRANGE("Document No.", "No.");
                IF DetailGSTLedgerEntry.FINDFIRST THEN BEGIN
                    REPEAT
                        IF DetailGSTLedgerEntry."GST Component Code" = 'CGST' THEN
                            TotAMTCGST += (-1 * DetailGSTLedgerEntry."GST Amount");
                        IF DetailGSTLedgerEntry."GST Component Code" = 'SGST' THEN
                            TotAMTSGST += (-1 * DetailGSTLedgerEntry."GST Amount");
                        IF DetailGSTLedgerEntry."GST Component Code" = 'IGST' THEN
                            TotAMTIGST += (-1 * DetailGSTLedgerEntry."GST Amount");

                    UNTIL DetailGSTLedgerEntry.NEXT = 0
                END;

                //TRI S.S 28.06.17 STOP
                //MESSAGE('%1   %2   %3   %4',"Amount to Customer",TotAMTCGST,TotAMTSGST,TotAMTIGST);
                // FinalAmt := "Amount to Customer"; //OA.AS Commented
                FinalAmt := Amount;
                //FinalAmt := "Amount to Customer" + TotAMTCGST + TotAMTIGST + TotAMTSGST;
