report 50018 "Sales Budget Vs Shipment"
{
    Caption = 'Sales Budget vs Shipment';
    ProcessingOnly = true;
        ...
        dataitem("Integer"; "Integer")
        {
            ...
            begin
                ...
                */
                    ...
                    REPEAT
                        ...
                        REPEAT
                            ...
                            CASE ItemRepeatCount OF
                                ...
                                6:
                                    EnterCell(
                                      RowNo, ColumnNo, H6, FALSE, FALSE, FALSE, '', TempExcelBuffer."Cell Type"::Text);
                            END;
                            ...
                            IF FindColumn('-') THEN
                                REPEAT
                                    ...
                                    CASE ItemRepeatCount OF
                                        ...
                                        2:
                                            BEGIN
                                                ColumnValue := CalcSalesValue(LineDimCodeBuffer.Code, ColumnDimCodeBuffer."Period Start", ColumnDimCodeBuffer."Period End");
                                            END;
                                            ...
                                            BEGIN
                                                ColumnValue :=
                                                              ItemBudgetManagement.CalcAmount(
                                                                ...
                                                                LineDimOption, LineDimCodeBuffer,
                                                                ColumnDimOption, ColumnDimCodeBuffer) -
                                                              CalcSalesValue(LineDimCodeBuffer.Code, ColumnDimCodeBuffer."Period Start", ColumnDimCodeBuffer."Period End");
                                        ...
                                        5:
                                            BEGIN
                                                ColumnValue := CalcSalesQty(LineDimCodeBuffer.Code, ColumnDimCodeBuffer."Period Start", ColumnDimCodeBuffer."Period End");
                                            END;
                                            ...
                                            BEGIN
                                                ColumnValue :=
                                                              ItemBudgetManagement.CalcAmount(
                                                                ...
                                                                LineDimOption, LineDimCodeBuffer,
                                                                ColumnDimOption, ColumnDimCodeBuffer) -
                                                                  CalcSalesQty(LineDimCodeBuffer.Code, ColumnDimCodeBuffer."Period Start", ColumnDimCodeBuffer."Period End");
                                            END;
                ...
                // END ELSE BEGIN
                //     TempExcelBuffer.CreateBook(ServerFileName, 'Sales Budget Vs Shipment');
    ...
    var
        ...
        H1: Label 'Sales Budget(Value)';
        H2: Label 'Actual Value';
        H3: Label 'Variance Value';
        H4: Label 'Sales Budget(Quantity)';
        H5: Label 'Actual Quantity';
        ProductionForecastEntry: Record "Production Forecast Entry";
        ItemLedgerEntry: Record "Item Ledger Entry";
        H6: Label 'Variance quantity';
    ...
    local procedure CalcSalesQty(ItemCode: Code[20]; StartDate: Date; EndDate: Date): Decimal
    var
        ItemLedgerEntry: Record "Item Ledger Entry";
    ...
    begin
        ...
        IF ItemLedgerEntry.FINDSET THEN
            REPEAT
                TotalSalesQty += ItemLedgerEntry.Quantity;
            UNTIL ItemLedgerEntry.NEXT = 0;

        EXIT(TotalSalesQty);
    end;

    local procedure CalcSalesValue(ItemCode: Code[20]; StartDate: Date; EndDate: Date): Decimal
    var
        ValueEntry: Record "Value Entry";
        TotalSalesValue: Decimal;
    begin
        CLEAR(TotalSalesValue);
        ValueEntry.RESET;
        ValueEntry.SETRANGE("Item No.", ItemCode);
        ValueEntry.SETRANGE("Posting Date", StartDate, EndDate);
        ValueEntry.SETRANGE("Item Ledger Entry Type", ValueEntry."Item Ledger Entry Type"::Sale);
        IF ValueEntry.FINDSET THEN
            REPEAT
                TotalSalesValue += ValueEntry."Cost Amount (Actual)";
            UNTIL ValueEntry.NEXT = 0;

        EXIT(TotalSalesValue);
    end;
}
