report 50014 "GST Purchase- Register"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Objects/RDLC/GSTPurchaseRegister.rdlc';

    dataset
    {
        dataitem("Detailed GST Ledger Entry"; "Detailed GST Ledger Entry")
        {
            DataItemTableView = SORTING("Transaction Type", "Document No.", "Posting Date") ORDER(Ascending) WHERE("Transaction Type" = CONST(Purchase));
            ...
            begin
                ...
                IF "Detailed GST Ledger Entry"."Source No." <> '' THEN BEGIN
                    IF Vend.GET("Detailed GST Ledger Entry"."Source No.") THEN BEGIN
                        ...
                        GSTIN := Vend."GST Registration No.";
                    END;
                END;
                CLEAR(SGST_);
                CLEAR(CGST_);
