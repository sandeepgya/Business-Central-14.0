report 50021 "Import Purchase Order_PMT"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Objects/RDLC/ImportPurchaseOrderPMT.rdlc';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(ShowTermsNConditions; ShowTermsNConditions)
            {
            }
            dataitem("Purchase Line"; "Purchase Line")
            {
                ...
                column(Tax_PurchaseLine; '')//"Purchase Line"."Tax %")
                {
                }
                column(ExcisePercent; ExcisePer)
                {
                }
                column(ExciseAmount_PurchaseLine; '')//"Excise Amount")
                {
                }
                column(TaxAmount_PurchaseLine; '')//"Tax Amount")
                {
                }
                column(AmountToVendor; '')//"Purchase Header"."Amount to Vendor")
                {
                }
                ...
                column(ServiceTaxAmt; '')//"Service Tax Amount")
                {
                }
            ...
            begin
                ...
                ShowTermsNConditions := (ContractCond[1] <> '') OR (ContractCond[2] <> '') OR (ContractCond[3] <> '');
                ...
                txtLocationDetails[6] := 'TIN No.:' + '';
                txtLocationDetails[7] := 'E.C.C.No.:' + '';
                //TRI VK GST
    ...
    var
        ...
        // StrOrderLineDetail: Record "Structure Order Line Details";
        EntryTaxAmt: Decimal;
